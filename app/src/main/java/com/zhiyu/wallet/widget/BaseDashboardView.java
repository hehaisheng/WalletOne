...
public abstract class BaseDashboardView extends View {
    ...
    // 默认边距
    private final static int DEFAULT_PADDING = 25;
    // 默认刻度文字
    private final static int[] DEFAULT_CALIBRATION_NUMBER = new int[]{0,5000,10000,15000,20000};
    private final static String[] DEFAULT_CALIBRATION_BETWEEN = new String[]{"","",""};//0  1W 2W
    // 默认刻度数量
    private final static int DEFAULT_LARGE_BETWEEN_CALIBRATION_NUMBER = 7;
    // 默认控件大小
    ...
    private final static int DEFAULT_VALUE_LEVEL_COLOR = Color.BLACK;
    // 时间画笔属性
    ...
    private void init(){
        ...
        mPaintDate.setTextSize(sp2px(19));
        mPaintDate.setColor(Color.parseColor("#323232"));
...
//        setValueLevelByInterval(index);
     ...
     * 重置角度信息
     */
    protected void resetCalibrationData(){
        //计算总共有多少个刻度
